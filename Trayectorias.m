...
% 2) Seleccion de trayectoria deseada (POSICIÓN y VELOCIDADES)
   ...
   % a)Trayectoria Churo     
        case 1   
            xd = 1*ones(1, length(t));         xd_p = 0*ones(1, length(t));     
            yd = 0*ones(1, length(t));         yd_p = 0*ones(1, length(t));     
            zd = 1*ones(1, length(t));         zd_p = 0*ones(1, length(t));
            % b) Trayectoria seno
        case 2 
